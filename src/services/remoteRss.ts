...
export const isOpmlUrl = (url: string) => /(?:\.(?:opml|opnl)(?:$|[?#]))|(?:[?&](?:format|type)=opml(?:&|$))/i.test(url.trim());

export type OpmlImportProgress = { total: number; done: number; imported: number; failed: number; currentTitle?: string };

type OpmlImportResult = { total: number; imported: number; failed: number };

export const importFeedsFromOpmlUrl = async (
  url: string,
  category: string,
  onProgress?: (value: OpmlImportProgress) => void,
): Promise<{ imported: number; failed: number }> => {
  onProgress?.({ total: 0, done: 0, imported: 0, failed: 0, currentTitle: url });
  const result = await apiRequest<OpmlImportResult>('/subscriptions/import-opml', {
    method: 'POST',
